...
/*
 * qrsolve2_helper: generate a random system of equations and solve
 */
static int qrsolve2_helper(double complex *x, double complex *a,
	double complex *b, int m, int n, int o)
{
    ...
    /*
     * Generate random matrices A and B, and a copy of A, U.
     */
    for (int i = 0; i < m; ++i) {
	for (int j = 0; j < n; ++j) {
    ...
    /*
     * Find the QR decomposition of U.
     */
     ...
     * Solve the system.
     */
    _vnacommon_qrsolve2(x, &q[0][0], &r[0][0], b, m, n, o);
...
#	    define NRHS 3
	...
	for (int m = 1; m <= 4; ++m) {
		...
		for (int o = 1; o <= 2; ++o) {
		     ...
		     * Generate random matrices A and B, and solve for X.
		     */
		    rank = qrsolve2_helper(&x[0][0], &a[0][0], &b[0][0],
			    m, n, o);
